        ...
        <main className="flex-1 overflow-y-auto p-6">
          ...
          {activeSection === 'rfq' && (
            <div className="space-y-5">
              ...
              <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
                {[
                  { label: 'Total RFQs', value: Object.values(rfqStats).reduce((a: any, b: any) => a + b, 0), color: '#3e3e3e', icon: FileText },
                  { label: 'Pending', value: rfqStats.Pending, color: '#F59E0B', icon: Clock },
                  { label: 'Quoted', value: rfqStats.Quoted, color: '#3B82F6', icon: MessageSquare },
                  { label: 'Accepted', value: rfqStats.Accepted, color: '#10B981', icon: CheckCircle2 },
                  { label: 'Rejected', value: rfqStats.Rejected, color: '#EF4444', icon: XCircle },
                ].map((s) => {
                  const Icon = s.icon;
                  return (
                    <div
              ...
              <div className="bg-white rounded-2xl shadow-sm border overflow-hidden" style={{ borderColor: '#E2E8F0' }}>
                ...
                <div className="flex border-b overflow-x-auto" style={{ borderColor: '#E2E8F0' }}>
                  {['all', 'Pending', 'Quoted', 'Accepted', 'Rejected'].map((status) => (
                    <button
                      ...
                      className="px-6 py-4 text-sm font-semibold whitespace-nowrap border-b-2 transition-colors"
                      style={{
                        borderColor: rfqStatusFilter === status ? '#ef4136' : 'transparent',
                        color: rfqStatusFilter === status ? '#ef4136' : '#64748B'
                      }}
                    >
                      {status === 'all' ? 'All RFQs' : status}
                      <span className="ml-2 px-2 py-0.5 rounded-full text-xs" style={{
                        backgroundColor: rfqStatusFilter === status ? '#FEF3C7' : '#F1F5F9',
                        color: rfqStatusFilter === status ? '#92400E' : '#64748B'
                      }}>
                        {status === 'all' ? Object.values(rfqStats).reduce((a: any, b: any) => a + b, 0) : (rfqStats as any)[status] || 0}
                      </span>
                    </button>
                  ))}
                </div>
                ...
                <div className="p-4 border-b flex flex-wrap gap-3 items-center" style={{ borderColor: '#E2E8F0' }}>
                  <div className="relative flex-1 min-w-64">
                    ...
                    <input
                      ...
                      placeholder="Search by RFQ ID, Buyer, or Product..."
                      className="w-full pl-9 pr-4 py-2.5 rounded-xl border text-sm outline-none"
                      style={{ borderColor: '#E2E8F0', backgroundColor: '#F8FAFC' }}
                    />
                  </div>
                ...
                <div className="divide-y" style={{ borderColor: '#F1F5F9' }}>
                  {rfqs.length > 0 ? rfqs.map((rfq: any) => {
                        ...
                        <div className="flex items-center justify-between gap-4">
                          <div className="flex-1 grid grid-cols-1 md:grid-cols-5 gap-4">
                            <div>
                              ...
                              <div className="text-xs mt-0.5" style={{ color: '#94A3B8' }}>{new Date(rfq.createdAt).toLocaleDateString()}</div>
                            </div>
                            <div>
                              <div className="text-xs mb-1" style={{ color: '#94A3B8' }}>Buyer</div>
                              <div className="font-semibold text-sm" style={{ color: '#1E293B' }}>{rfq.user?.fullName}</div>
                              <div className="text-xs mt-0.5" style={{ color: '#94A3B8' }}>{rfq.user?.phone}</div>
                            </div>
                            <div>
                              <div className="text-xs mb-1" style={{ color: '#94A3B8' }}>Product Details</div>
                              <div className="font-semibold text-sm" style={{ color: '#1E293B' }}>{rfq.product?.title || rfq.serviceGig?.title || 'Custom RFQ'}</div>
                              <div className="text-xs mt-0.5" style={{ color: '#94A3B8' }}>Qty: {rfq.requiredQuantity}</div>
                            </div>
                            <div>
                              <div className="text-xs mb-1" style={{ color: '#94A3B8' }}>Delivery</div>
                              <div className="text-sm truncate max-w-[150px]" style={{ color: '#1E293B' }} title={rfq.deliveryLocation}>{rfq.deliveryLocation}</div>
                            </div>
                            <div>
                              <div className="text-xs mb-1" style={{ color: '#94A3B8' }}>Status</div>
                              ...
                              <div className="text-xs mt-1 flex items-center gap-1" style={{ color: '#94A3B8' }}>
                                <MessageSquare size={11} /> {rfq.responses?.length || 0} msg
                              </div>
                            </div>
                          </div>
                          ...
                          <div className="flex items-center gap-2">
                            <button
                              ...
                              style={{
                                backgroundColor: rfq.status === 'Pending' ? '#ef4136' : '#3B82F6',
                                color: 'white'
                              }}
                            >
                              {rfq.status === 'Pending' ? (
                    ...
                    <div className="p-12 text-center">
                      ...
                      <p className="text-sm" style={{ color: '#94A3B8' }}>There are no RFQs matching your criteria.</p>
                    </div>
                  )}
                </div>
                ...
                {rfqLastPage > 1 && (
                  <div className="p-4 border-t flex items-center justify-between" style={{ borderColor: '#E2E8F0' }}>
                    <div className="text-sm" style={{ color: '#64748B' }}>
                      Showing page <span className="font-bold text-gray-900">{rfqPage}</span> of <span className="font-bold text-gray-900">{rfqLastPage}</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <button
                        onClick={() => setRfqPage(Math.max(1, rfqPage - 1))}
                        disabled={rfqPage === 1}
                        className="px-4 py-2 rounded-xl border text-sm font-bold disabled:opacity-40 hover:bg-gray-50 transition-colors"
                        style={{ borderColor: '#E2E8F0', color: '#64748B' }}
                      >
                        Previous
                      </button>
                      <button
                        onClick={() => setRfqPage(Math.min(rfqLastPage, rfqPage + 1))}
                        disabled={rfqPage === rfqLastPage}
                        className="px-4 py-2 rounded-xl border text-sm font-bold disabled:opacity-40 hover:bg-gray-50 transition-colors"
                        style={{ borderColor: '#E2E8F0', color: '#64748B' }}
                      >
                        Next
              ...
              {selectedRFQ && (
                <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4" onClick={() => setSelectedRFQ(null)}>
                  <div className="bg-white rounded-2xl shadow-xl max-w-3xl w-full max-h-[90vh] overflow-auto" onClick={(e) => e.stopPropagation()}>
                    {isLoadingRFQDetails ? (
                      <div className="p-12 text-center">
                        <RefreshCw size={48} className="mx-auto mb-4 animate-spin text-orange-500" />
                        <p className="font-semibold text-lg" style={{ color: '#64748B' }}>Loading RFQ Details...</p>
                      </div>
                    ) : selectedRFQData ? (
                      <div className="bg-white rounded-2xl">
                        ...
                        <div className="p-5 border-b" style={{ borderColor: '#E2E8F0' }}>
                          <div className="flex items-start justify-between mb-3">
                            <div>
                              <h3 className="font-bold text-lg mb-1" style={{ color: '#1E293B' }}>RFQ #{selectedRFQData.id}</h3>
                              <p className="text-sm" style={{ color: '#64748B' }}>
                                {selectedRFQData.product?.title || selectedRFQData.serviceGig?.title || 'Custom Request'} • Qty: {selectedRFQData.requiredQuantity}
                              </p>
                            </div>
                            <div className="flex items-center gap-2">
                              {(() => {
                              ...
                              <button onClick={() => setSelectedRFQ(null)} className="p-1 hover:bg-gray-100 rounded-full">
                                <X size={20} style={{ color: '#64748B' }} />
                              </button>
                            </div>
                          </div>
                          ...
                          <div className="grid grid-cols-2 gap-4 p-4 rounded-xl" style={{ backgroundColor: '#F8FAFC' }}>
                            <div>
                              <div className="text-xs mb-1" style={{ color: '#94A3B8' }}>Buyer</div>
                              <div className="font-semibold text-sm" style={{ color: '#1E293B' }}>{selectedRFQData.user?.fullName}</div>
                              <div className="text-xs mt-1 flex items-center gap-1" style={{ color: '#64748B' }}>
                                <Phone size={11} /> {selectedRFQData.user?.phone}
                              </div>
                            </div>
                            <div>
                              <div className="text-xs mb-1" style={{ color: '#94A3B8' }}>Delivery Details</div>
                              <div className="text-sm font-medium" style={{ color: '#1E293B' }}>{selectedRFQData.deliveryLocation}</div>
                              <div className="text-xs mt-1" style={{ color: '#64748B' }}>
                                Requested on: {new Date(selectedRFQData.createdAt).toLocaleDateString()}
                              </div>
                            </div>
                          </div>
                        </div>
                        ...
                        {/* Requirements */}
                        <div className="p-5 border-b" style={{ borderColor: '#E2E8F0' }}>
                          <h4 className="text-xs font-bold uppercase tracking-wider mb-2" style={{ color: '#94A3B8' }}>Additional Requirements</h4>
                          <p className="text-sm" style={{ color: '#334155' }}>{selectedRFQData.additionalRequirement || 'No additional requirements provided.'}</p>
                        </div>

                        {/* Reply Form */}
                        {selectedRFQData.status === 'Pending' && (
                          <div className="p-5 border-t" style={{ borderColor: '#E2E8F0' }}>
                            <h4 className="font-semibold mb-3" style={{ color: '#1E293B' }}>Send Status Update</h4>

                            <div className="flex items-center gap-3">
                              <button
                                onClick={() => handleUpdateQuotationStatus(selectedRFQData.id, 'Quoted')}
                                disabled={updateQuotationStatusMutation.isPending}
                                className="flex-1 py-3 rounded-xl text-white font-semibold flex items-center justify-center gap-2 hover:opacity-90 transition-opacity disabled:opacity-50"
                                style={{ backgroundColor: '#3B82F6' }}
                              >
                                {updateQuotationStatusMutation.isPending ? <RefreshCw size={18} className="animate-spin" /> : <Send size={18} />}
                                Mark as Quoted
                              </button>
                              <button
                                onClick={() => handleUpdateQuotationStatus(selectedRFQData.id, 'Rejected')}
                                disabled={updateQuotationStatusMutation.isPending}
                                className="flex-1 py-3 rounded-xl border font-semibold flex items-center justify-center gap-2 hover:bg-gray-50 transition-colors disabled:opacity-50"
                                style={{ borderColor: '#EF4444', color: '#EF4444' }}
                              >
                                {updateQuotationStatusMutation.isPending ? <RefreshCw size={18} className="animate-spin" /> : <XCircle size={18} />}
                                Reject Request
                              </button>
                            </div>
                          </div>
                        ...
                        {selectedRFQData.status === 'Accepted' && (
                          <div className="p-5 border-t flex items-center gap-3" style={{ borderColor: '#E2E8F0', backgroundColor: '#F0FDF4' }}>
                            ...
                            <div>
                              ...
                              <p className="text-xs" style={{ color: '#15803D' }}>The buyer has accepted your quotation. Proceed to process the order.</p>
                            </div>
                          </div>
                        )}
                        ...
                        {selectedRFQData.status === 'Rejected' && (
                          <div className="p-5 border-t flex items-center gap-3" style={{ borderColor: '#E2E8F0', backgroundColor: '#FEF2F2' }}>
                            ...
                            <div className="flex-1">
                              <p className="font-semibold text-sm" style={{ color: '#991B1B' }}>Request Rejected</p>
                              <p className="text-xs" style={{ color: '#B91C1C' }}>This RFQ has been rejected.</p>
                            </div>
                          </div>
                        )}
                      </div>
                      ...
                      <div className="p-12 text-center">
                        ...
                        <p className="font-semibold text-lg" style={{ color: '#64748B' }}>Quotation not found</p>
                      </div>
                    )}
                  </div>
                </div>
